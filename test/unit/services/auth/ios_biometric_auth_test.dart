...
/// Tests for iOS Face ID and Touch ID authentication.
/// Note: These tests use mocks to simulate iOS biometric behavior.
/// Actual device testing is required for full verification.
void main() {
  group('iOS Face ID authentication works correctly', () {
    ...
    test('Face ID can be detected on device', () async {
    ...
    test('biometric login can be enabled after Face ID enrollment', () async {
      when(mockLocalAuth.canCheckBiometrics).thenAnswer((_) async => true);
      when(mockLocalAuth.isDeviceSupported()).thenAnswer((_) async => true);
      when(mockLocalAuth.getAvailableBiometrics())
          .thenAnswer((_) async => [BiometricType.face]);
    ...
    test('Face ID authentication restores session', () async {
      when(mockLocalAuth.authenticate(
        localizedReason: anyNamed('localizedReason'),
        options: anyNamed('options'),
      )).thenAnswer((_) async => true);
  ...
  group('iOS Touch ID authentication works correctly', () {
    late ProviderContainer container;
    late MockFlutterSecureStorage mockStorage;
    late MockLocalAuthentication mockLocalAuth;
    ...
    test('Touch ID can be detected on device', () async {
      when(mockLocalAuth.canCheckBiometrics).thenAnswer((_) async => true);
      when(mockLocalAuth.isDeviceSupported()).thenAnswer((_) async => true);
      when(mockLocalAuth.getAvailableBiometrics())
    ...
    test('Touch ID authentication restores session', () async {
      when(mockLocalAuth.authenticate(
        localizedReason: anyNamed('localizedReason'),
        options: anyNamed('options'),
      )).thenAnswer((_) async => true);
  ...
  group('iOS Keychain stores credentials securely', () {
    late ProviderContainer container;
    late MockFlutterSecureStorage mockStorage;
    ...
    test('secureStorageProvider is configured with iOS Keychain options', () {
    ...
    test('tokens are stored securely on login', () async {
      when(mockStorage.write(
        key: anyNamed('key'),
        value: anyNamed('value'),
      )).thenAnswer((_) async {});
